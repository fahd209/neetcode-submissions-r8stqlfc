class Solution:
    def characterReplacement(self, s: str, k: int) -> int:
        ...
        while right < len(s):
            ...
            # if the character replacement in the cur window is greater then k
            # keep on reducing the window until the char replacement is equal to k
            while ((right - left + 1) - most_repeating_char_len) > k and left < right: 
                charCount[s[left]] -= 1
                left += 1

                # keep track of the most_repeating_char while reducing the window 
